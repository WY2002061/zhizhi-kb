  ...
  const draw = useCallback(() => {
    ...
    for (const node of nodesRef.current) {
      ...
      if (node.type === 'collection') {
        ...
        ctx.fillStyle = isHovered ? '#7A8D9E' : '#8B9DAF';
        ctx.shadowColor = '#8B9DAF40';
        ctx.shadowBlur = isHovered ? 12 : 4;
      } else {
        ctx.fillStyle = isHovered ? '#8FAA85' : '#A5B89C';
        ctx.shadowBlur = 0;
      }
      ...
      if (node.type !== 'tag' || isHovered) {
        ctx.fillStyle = node.type === 'tag' ? '#4A5D42' : '#1E293B';
  ...
  return (
    <div className="space-y-6 animate-fadeIn pb-10 h-full flex flex-col">
      <div className="flex items-center justify-between flex-shrink-0">
        <div>
          <h1 className="text-2xl font-semibold text-stone-800 tracking-tight">知识图谱</h1>
          <p className="text-base text-stone-500 mt-1.5 font-medium">可视化你的知识关联网络</p>
        </div>
        <div className="flex items-center gap-1 bg-white p-1 rounded-lg border border-stone-200">
          <button onClick={() => setZoom((z) => Math.min(3, z * 1.2))} className="p-2 rounded-md text-stone-500 hover:bg-stone-50 hover:text-stone-700 transition-colors">
            <ZoomIn size={17} />
          </button>
          <button onClick={() => setZoom((z) => Math.max(0.3, z * 0.8))} className="p-2 rounded-md text-stone-500 hover:bg-stone-50 hover:text-stone-700 transition-colors">
            <ZoomOut size={17} />
          </button>
          <button onClick={() => { setZoom(1); setOffset({ x: 0, y: 0 }); }} className="p-2 rounded-md text-stone-500 hover:bg-stone-50 hover:text-stone-700 transition-colors">
            <Maximize2 size={17} />
          </button>
        </div>
      </div>

      <div className="flex items-center gap-5 text-sm text-stone-500 font-medium flex-shrink-0 bg-white px-5 py-2.5 rounded-lg border border-stone-200 w-fit">
        <span className="flex items-center gap-2"><span className="w-3 h-3 rounded-full inline-block" style={{ backgroundColor: '#8B9DAF' }} /> 知识卡片</span>
        <span className="flex items-center gap-2"><span className="w-3 h-3 rounded-full inline-block" style={{ backgroundColor: '#A5B89C' }} /> 标签</span>
        <span className="flex items-center gap-2"><span className="w-3 h-3 rounded-full bg-rose-400 inline-block" /> 知识集</span>
        <span className="flex items-center gap-2 text-stone-400 ml-4 border-l border-stone-200 pl-5"><Info size={15} /> 双击卡片节点查看详情</span>
      </div>

      <div ref={containerRef} className="flex-1 bg-white rounded-lg border border-stone-200 overflow-hidden relative min-h-[500px]">
        <canvas
          ref={canvasRef}
        ...
        />
        {hoveredNode && (
          <div className="absolute bottom-5 left-5 bg-white/95 backdrop-blur-sm border border-stone-200 rounded-lg px-4 py-2.5 shadow-sm animate-fadeIn">
            <p className="text-base font-medium text-stone-800">{hoveredNode.label}</p>
            <p className="text-sm text-stone-500 capitalize mt-0.5">{hoveredNode.type === 'card' ? '知识卡片' : hoveredNode.type === 'tag' ? '标签' : '知识集'}</p>
          </div>
        )}
      </div>
    </div>
